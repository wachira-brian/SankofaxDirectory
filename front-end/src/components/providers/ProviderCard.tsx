...
const ProviderCard: React.FC<ProviderCardProps> = ({ provider }) => {
  return (
    <Card className="h-full transition-transform duration-300 hover:shadow-lg hover:-translate-y-1">
      <Link to={`/provider/${provider.id}`} className="block h-full"> {/* Changed to /provider/:providerId */}
        <div className="relative">
          ...
          />
          {/* Optional Diaspora Tag (if data exists) */}
        ...
        <div className="p-4">
          ...
          <p className="text-gray-600 text-sm line-clamp-2 mb-3">{provider.description}</p>
